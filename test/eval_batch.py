...
def parse_args() -> Namespace:
    ...
    p.add_argument("--datalen", type=str, default=40*1024)
...
if __name__ == '__main__':
    ...
    sparse_budget = length // 160
    temperature = 1.0
    baseline_bsz = 12  # baseline batchsize
    cv_bsz = 28  # catekv batchsize
    ...
    dataset = Dataset(dataset_name, llm_baseline.tokenizer, 128*1024, 96)
    if baseline_bsz == 1:
        ...
        input_ids = torch.cat([dataset[i][0][:, :length] for i in range(llm_baseline.batch_size)], dim=0)
    ...
    dataset = Dataset(dataset_name, llm_cv.tokenizer, 128 * 1024, 96)
    if cv_bsz == 1:
        total_sample_needed = length // dataset_maxlen
        remaining_length = length % dataset_maxlen
        input_ids_list = []
        ...
        input_ids = torch.cat([dataset[i][0][:, :length] for i in range(llm_cv.batch_size)], dim=0)

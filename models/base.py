...
class LLM:
    ...
    def layer_compute(self, 
        ...
        if isinstance(self.kv_cache, KV_Cache):
            ...
            if q_len > 4*1024: # prefill
                ...
                # flash attention
                hidden_states = flash_attn_with_kvcache(q=query_states.transpose(1, 2), k_cache=key_states.transpose(1, 2), v_cache=value_states.transpose(1, 2), causal=True)
        elif isinstance(self.kv_cache, CateKV_dynamic):
            if q_len > 4*1024: # prefill
                query_states, key_states = self.apply_rotary_pos_emb(query_states, key_states, position_ids)
                ...
                # get retrieval idx
                head_type, key_states_selected, value_states_selected,key_states_all,value_states_all = self.kv_cache.collect_kv(layer_idx=layer_idx, query_states=query_states)
                head_num_in_groups = query_states.shape[1] // key_states_selected.shape[1]
                repeated_head_type = torch.repeat_interleave(head_type, repeats=head_num_in_groups)
                full_head_query_index = torch.nonzero(repeated_head_type == 0).flatten()
                sparse_head_query_index = torch.nonzero(repeated_head_type == 1).flatten()
                full_head_key_index = torch.nonzero(head_type == 0).flatten()
                sparse_head_key_index = torch.nonzero(head_type == 1).flatten()

                if full_head_query_index.numel() > 0:
                    full_query_states = query_states[:,full_head_query_index,:,:]
                    full_key_states = key_states_all[:, full_head_key_index, :, :]
                    full_value_states = value_states_all[:, full_head_key_index, :, :]
                    full_hidden_states = flash_attn_with_kvcache(
                        q=full_query_states.transpose(1, 2),
                        k_cache=full_key_states.transpose(1, 2),
                        v_cache=full_value_states.transpose(1, 2),
                        causal=True
                    )
                else:
                    full_hidden_states = torch.zeros_like(query_states.transpose(1,2))

                if sparse_head_query_index.numel() > 0:
                    sparse_query_states = query_states[:,sparse_head_query_index,:,:]
                    sparse_key_states = key_states_selected[:, sparse_head_key_index, :, :]
                    sparse_value_states = value_states_selected[:, sparse_head_key_index, :, :]
                    sparse_hidden_states = flash_attn_with_kvcache(
                        q=sparse_query_states.transpose(1, 2),
                        k_cache=sparse_key_states.transpose(1, 2),
                        v_cache=sparse_value_states.transpose(1, 2),
                        causal=True
                    )
                else:
                    sparse_hidden_states = torch.zeros_like(query_states.transpose(1,2))
                hidden_states = torch.zeros_like(query_states.transpose(1,2))
                if full_head_query_index.numel() > 0:
                    hidden_states[:, :, full_head_query_index, :] = full_hidden_states
                if sparse_head_query_index.numel() > 0:
                    hidden_states[:, :, sparse_head_query_index, :] = sparse_hidden_states
        elif isinstance(self.kv_cache, CateKV):
            if q_len > 4*1024: # prefill
                query_states, key_states = self.apply_rotary_pos_emb(query_states, key_states, position_ids)
                self.kv_cache.prefill_kv_cache(query_states,key_states, value_states, layer_idx,None)
                ...
                # get retrieval idx
                full_head_query_index,sparse_head_query_index, full_k, full_v, sparse_k, sparse_v = self.kv_cache.collect_kv(layer_idx=layer_idx, query_states=query_states)
                if full_head_query_index.numel() > 0:
                    full_query_states = query_states[:,full_head_query_index,:,:]
                    full_hidden_states = flash_attn_with_kvcache(
                        q=full_query_states.transpose(1, 2),
                        k_cache=full_k.transpose(1, 2),
                        v_cache=full_v.transpose(1, 2),
                        causal=True
                    )
                else:
                    full_hidden_states = torch.zeros_like(query_states.transpose(1,2))
                ...
                if sparse_head_query_index.numel() > 0:
                    ...
                    sparse_hidden_states = flash_attn_with_kvcache(
                        q=sparse_query_states.transpose(1, 2),
                        k_cache=sparse_k.transpose(1, 2),
                        v_cache=sparse_v.transpose(1, 2),
                        causal=True
                    )
                else:
                    sparse_hidden_states = torch.zeros_like(query_states.transpose(1,2))
    ...
    def batch_prefill(self, input_ids: torch.Tensor, benchmark: bool = False):
        ...
        for bsz in tqdm(range(0, batch_size, T), desc=f"Prefilling (batch size={batch_size})"):
            ...
            logits[bsz:bsz+T].copy_(self.inference(input_ids=req_input_ids, position_ids=self.get_ctx(req_input_ids))[0])
            gc.collect()
            torch.cuda.empty_cache()
            torch.cuda.synchronize()
        assert self.kv_cache.get_kv_len() == input_ids.shape[-1], f"KV length mismatch, got {self.kv_cache.get_kv_len()}, expected {input_ids.shape[-1]}"
    ...
    def batch_generate(self, input_ids: torch.Tensor, gen_len: int = 256, temperature: float = 0.0, top_p: float = -1, top_k :int = 50, verbose: bool = False, benchmark: bool = False, cont: bool = False):
        ...
        if benchmark == True:
            ...
            print(f"\nPrefill {input_ids.size(1)} tokens | Decoding time in generating {n} tokens is {round(end - start, 4)}s | Throughput: {round(self.batch_size * n / (end - start), 2)} tokens/s, Latency: {round((end - start)*1000 / n, 2)} ms/step | Peak Memory {round(peak_memory, 2)}\n")
        ...
        if benchmark == True:
            return self.decode(generated_ids, skip_special_tokens=True), self.batch_size * n / (end - start), peak_memory

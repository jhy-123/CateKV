...
from .tensor_op import layer_norm, apply_rotary_pos_emb, apply_rotary_pos_emb_single, sample_token,apply_rotary_pos_emb_yarn,apply_rotary_pos_emb_single_yarn
...
class Qwen2(LLM):
    def __init__(self, 
        ...
        head_classification_path: str = 'path/to/head_classification/path',
        batch_size :int = 1,
        ...
        if 'rope_scaling' not in self.config.to_dict():
        ...
        _, self.attention_scaling = self.rope_init_fn(self.config, self.device)
        ...
        self.init_kv_cache(head_classification_path,sparse_budget, rank, chunk_size, last_q, init_tokens, recent_tokens, top_threshold, top_fraction, cv_threshold, full_fraction, self.config)
    ...
    def apply_rotary_pos_emb_single(self, x: torch.Tensor, position_ids: torch.Tensor) -> torch.Tensor:
        return apply_rotary_pos_emb_single_yarn(x, self.cos_cache, self.sin_cache, position_ids,self.attention_scaling)

    @torch.inference_mode()
    def apply_rotary_pos_emb(self, q: torch.Tensor, k: torch.Tensor, position_ids: torch.Tensor) -> torch.Tensor:
        return apply_rotary_pos_emb_yarn(q, k, self.cos_cache, self.sin_cache, position_ids,self.attention_scaling)

...
def apply_rotary_pos_emb_yarn(q, k, cos, sin, position_ids, attention_scaling):
    cos = cos[position_ids].unsqueeze(1) * attention_scaling
    sin = sin[position_ids].unsqueeze(1) * attention_scaling
    q_embed = (q * cos) + (rotate_half(q) * sin)
    k_embed = (k * cos) + (rotate_half(k) * sin)
    return q_embed, k_embed

def apply_rotary_pos_emb_single_yarn(q, cos, sin, position_ids, attention_scaling, unsqueeze_dim=1):
    # if position_ids shape is (batch_size, num_heads, seq_len), then reshape it to (batch_size*num_heads, seq_len)
    if len(position_ids.shape) == 3:
        position_ids = position_ids.view(-1, position_ids.size(-1))
        cos = cos[position_ids] * attention_scaling
        sin = sin[position_ids] * attention_scaling
        q_embed = (q * cos) + (rotate_half(q) * sin)

    else:
        cos = cos[position_ids].unsqueeze(unsqueeze_dim) * attention_scaling
        sin = sin[position_ids].unsqueeze(unsqueeze_dim) * attention_scaling
        q_embed = (q * cos) + (rotate_half(q) * sin)
    return q_embed
